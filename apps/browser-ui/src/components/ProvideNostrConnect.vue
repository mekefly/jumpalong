<script lang="ts" setup>
import { AskOption } from '@jumpalong/nostr'
import { Pubkey } from '@jumpalong/nostr-shared'
import Drawer from './Drawer.vue'
import Papaw from './Papaw.vue'
import { useEventLine, usePubkey } from './ProvideEventLine'
import { provideSigner } from './ProvideNostrConnect'
import PubkeyLink from './PubkeyLink.vue'
import Scrollbar from './Scrollbar.vue'

const pubkey = usePubkey()
const line = useEventLine()
let { remoteSigner } = provideSigner()

const connectLine = computed(() => {
  if (pubkey.value) {
    return remoteSigner.provideConnect()
  } else {
    return
  }
})
const set = ref<Set<string>>(new Set())
const reqList = ref<AskOption[]>([])
const opt = ref<undefined | AskOption>()
watchEffect(() => {
  connectLine.value?.onAsk((_opt: AskOption) => {
    const id = _opt.requestOpt.id
    if (set.value.has(id)) return
    set.value.add(_opt.requestOpt.id)

    reqList.value.push(_opt)
  })
})
watch([() => reqList.value.length, opt], () => {
  if (opt.value === undefined) {
    let _opt = reqList.value.pop()
    if (!_opt) return
    remoteSigner.autoAsk(_opt, () => {
      opt.value = _opt
    })
  }
})
const duration = ref<undefined | number>()
function handleAllow() {
  opt.value?.allow(duration.value)
  opt.value = undefined
}
function handleCancellation() {
  opt.value?.refuse(duration.value)
  opt.value = undefined
}

function getDecryptValue(pubkey: string, ciphertext: string) {
  return asyncComputed(
    async () => await line.nostrApi.decrypt(Pubkey.fromHex(pubkey), ciphertext)
  )
}
</script>

<template>
  <Drawer :show="Boolean(opt)" :placement="'top'" :height="400" closable>
    <template #header>
      {{ opt && t(opt?.requestOpt.method) }}
      <span v-if="reqList.length > 0"> ({{ reqList.length }}) </span>
    </template>
    <template #default>
      <Scrollbar>
        <div v-if="opt">Request ID: {{ opt.requestOpt.id }}</div>
        <div v-if="opt">{{ opt.event.pubkey.slice(0, 10) }}</div>
        <div v-if="opt?.requestOpt.method === 'describe'">
          {{ t('want_to_obtain_descriptive_information') }}
        </div>

        <div v-if="opt?.requestOpt.method === 'connect'">
          {{ t('want_to_establish_a_connection_with_you') }}
        </div>

        <div v-if="opt?.requestOpt.method === 'get_relays'">
          {{ t('want_to_obtain_your_relays') }}
        </div>

        <div v-if="opt?.requestOpt.method === 'get_public_key'">
          {{ t('want_to_obtain_your_public_key') }}
        </div>

        <div v-if="opt?.requestOpt.method === 'nip04_encrypt'">
          {{ t('want_to_encrypt_the_following_content') }}

          <p>
            {{ t('send_to') }}<PubkeyLink :pubkey="opt.requestOpt.params[0]" />
          </p>
          <p>{{ opt.requestOpt.params[1] }}</p>
        </div>
        <div v-if="opt?.requestOpt.method === 'nip04_decrypt'">
          {{ t('want_to_decrypt_the_following_content') }}

          <p>
            {{ t('sender') }}<PubkeyLink :pubkey="opt.requestOpt.params[0]" />
          </p>
          <p>{{ getDecryptValue(...opt.requestOpt.params) }}</p>
        </div>

        <div v-if="opt?.requestOpt.method === 'sign_event'">
          {{ t('want_to_sign_the_following_event') }}
          <Papaw :event="(opt.requestOpt.params[0] as any)"></Papaw>
        </div>

        <p class="mt-4">
          {{ t('not_asking_anymore') }}
        </p>
        <n-radio-group v-model:value="duration" name="radiogroup">
          <n-space>
            <n-radio
              v-for="item in [
                [null, '一次'],
                [15 * 60 * 1000, `15${t('minutes')}`],
                [24 * 3600 * 1000, `1${t('day')}`],
                [7 * 24 * 3600 * 1000, `7${t('day')}`],
                [30 * 24 * 3600 * 1000, `30${t('day')}`],
              ]"
              :key="item[0]"
              :value="item[0]"
            >
              {{ item[1] }}
            </n-radio>
          </n-space>
        </n-radio-group>
      </Scrollbar>
    </template>
    <template #footer>
      <div class="flex w-full">
        <NSpace class="w-full" vertical>
          <n-space class="">
            <n-button @click="handleAllow">{{ t('allow') }} </n-button>
            <n-button @click="handleCancellation">{{ t('refuse') }}</n-button>
          </n-space>
        </NSpace>
      </div>
    </template>
  </Drawer>

  <slot></slot>
</template>

<style scoped></style>
