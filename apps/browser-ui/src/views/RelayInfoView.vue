...
function fetch1(url: string) {
  ...
  return fetch(`${url}`, initOption).catch(() => {
    isProxy.value = true
    return fetch(`${proxyurl}${url}`, initOption)
  })
}
async function req(url: string) {
  // const proxyurl = "https://secret-ocean-49799.herokuapp.com/";
  return await useCache(
    `fetch:${url}:get:application/nostr+json`,
    async () => {
