...
import { deserializeTagR } from '../event/tag'
...
export function decodeToPrikey(anyPrikey: string) {
  ...
  if (isPrikey(anyPrikey)) {
    return Prikey.fromHex(anyPrikey)
  }
  try {
    const decodeData: any = nip19.decode(anyPrikey)
    if (decodeData.type === 'nsec') {
      return Prikey.fromHex(decodeData.data)
    } else {
      return null
    }
  } catch (error) {
...
export function isPrikey(prikey: string): boolean {
  try {
    const nsec = nip19.nsecEncode(Prikey.fromHex(prikey))
    return Prikey.fromUint8Array(nip19.decode(nsec).data).toHex() === prikey
  } catch (error) {
    return false
  }
}
